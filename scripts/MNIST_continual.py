    ...
    args = {
       "-path_to_dataset": {
        ...
        "-save_each_iteration": {
            "default": False,
            "type": bool,
            "help": "save model, loss and opt state each iteration"
        }
    }
...
if __name__ == "__main__":
    ...
    header += ",hash,final_loss,model_size,train_accuracy,test_accuracy,training_time,digits_up_to"
    ...
    keys = random.split(random.PRNGKey(args["key"]))
    ...
    mask = jnp.zeros(targets.shape, dtype=bool)
    key = keys[1]
    for digits_up_to in range(args["N_classes"]):
        keys = random.split(key, 5)
        key = keys[0]
        mask = jnp.logical_or(mask, targets == digits_up_to)
        sc = optax.exponential_decay(args["learning_rate"], args["N_drop"], args["gamma"])
        optim = optax.lion(learning_rate=sc)
        opt_state = optim.init(eqx.filter(model, eqx.is_array))
    
        inds = random.choice(keys[1], features[:args["N_train"]][mask[:args["N_train"]]].shape[0], (args["N_updates"], args["N_batch"]))
        carry = [model, features[:args["N_train"]][mask[:args["N_train"]]], targets[:args["N_train"]][mask[:args["N_train"]]], opt_state, keys[2]]
        ...
        _, is_correct = scan(lambda a, b: transient_learning.compute_error(a, b, args["N_t"]), [model, features[:args["N_train"]][mask[:args["N_train"]]], targets[:args["N_train"]][mask[:args["N_train"]]], keys[3]], jnp.arange(features[:args["N_train"]][mask[:args["N_train"]]].shape[0]))
        train_accuracy = jnp.sum(is_correct) / is_correct.shape[0]
        _, is_correct = scan(lambda a, b: transient_learning.compute_error(a, b, args["N_t"]), [model, features[args["N_train"]:][mask[args["N_train"]:]], targets[args["N_train"]:][mask[args["N_train"]:]], keys[4]], jnp.arange(features[args["N_train"]:][mask[args["N_train"]:]].shape[0]))
        test_accuracy = jnp.sum(is_correct) / is_correct.shape[0]
        ...
        if args["save_each_iteration"]:
            eqx.tree_serialise_leaves(f"{args['path_to_results']}/model_{digits_up_to}_{exp_hash}.eqx", model)
            eqx.tree_serialise_leaves(f"{args['path_to_results']}/opt_state_{digits_up_to}_{exp_hash}.eqx", opt_state)
            jnp.save(f"{args['path_to_results']}/learning_curve_{digits_up_to}_{exp_hash}.npy", losses)
    
        data = "\n" + ",".join([str(args[key]) for key in args.keys()])
        data += f",{exp_hash},{losses[-1]},{model_size},{train_accuracy},{test_accuracy},{training_time},{digits_up_to}"
    
        with open(f"{args['path_to_results']}/results.csv", "a") as f:
            f.write(data)
    if not args["save_each_iteration"]:
        eqx.tree_serialise_leaves(f"{args['path_to_results']}/model_{digits_up_to}_{exp_hash}.eqx", model)
        eqx.tree_serialise_leaves(f"{args['path_to_results']}/opt_state_{digits_up_to}_{exp_hash}.eqx", opt_state)
        jnp.save(f"{args['path_to_results']}/learning_curve_{digits_up_to}_{exp_hash}.npy", losses)